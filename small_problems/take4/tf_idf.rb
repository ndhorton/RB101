...
=begin
The problem is with the last line of the `idf` function
if no document contains the term, either a division by zero error
  will occur
To fix this it is necessary to put some kind of guard against
  doing the division if the divisor is zero

The actual problem however is that we are doing integer division
