# TF-IDF
# The problem was that the `idf` method was doing integer division
# when it should have been doing float division in the expression
# passed as argument to the `Math::log` method
# Further explorations seems to be hinting at potential zero division
# error if no document contains the term.
# This is correct although it's worth noting that here you
# get NaN/Infinity results instead of a ZeroDivisionError
# being raised (as it would be if we were still doing integer
# division) which can make things harder to debug in that
# your program doesn't raise an exception.

# LS solution uses `Math::fdiv` instead of `Integer#to_f` to
