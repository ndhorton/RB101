...
def declare_game_winner(player_score, dealer_score)
  ...
  prompt "Player has won " + hand_or_hands(player_score)
  prompt "Dealer has won " + hand_or_hands(dealer_score)
  puts
  if player_score > dealer_score
...
loop do
  ...
  loop do
    ...
    display_score(get_score(player_hand),
                  "#{dealer_partial_score} + ?")
